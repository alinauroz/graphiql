...
(function() {
  ...
  var Vim = function() {
    ...
    const numberRegex = /[\d]/;
    const wordRegexp = [/\w/, /[^\w\s]/];
    const bigWordRegexp = [/\S/];
    ...
    const validMarks = [].concat(
      upperCaseAlphabet,
      lowerCaseAlphabet,
      numbers,
      ['<', '>'],
    );
    const validRegisters = [].concat(
      upperCaseAlphabet,
      lowerCaseAlphabet,
      numbers,
      ['-', '"', '.', ':'],
    ...
    function setOption(name, value) {
      ...
      option.value = option.type == 'boolean' ? Boolean(value) : value;
    }
    ...
    const createCircularJumpList = function() {
      ...
      function add(cm, oldCur, newCur) {
        ...
        if (curMark) {
          const markPos = curMark.find(); // avoid recording redundant cursor position
      ...
      function move(cm, offset) {
        ...
        let mark = buffer[(size + pointer) % size]; // skip marks that are temporarily removed from text buffer
    ...
    function Register(text, linewise) {
      ...
      this.searchQueries = [];
      this.linewise = Boolean(linewise);
    }

    Register.prototype = {
      setText(text, linewise) {
        this.keyBuffer = [text || ''];
        this.linewise = Boolean(linewise);
      },
    ...
    RegisterController.prototype = {
      pushText(registerName, operator, text, linewise) {
        ...
        const register = this.isValidRegister(registerName)
          ...
          : null; // if no register/an invalid register was specified, things go to the
        // default registers
        ...
        if (append) {
          register.append(text, linewise); // The unnamed register always has the same value as the last used
          // register.
      ...
      shiftNumericRegisters_() {
        for (let i = 9; i >= 2; i--) {
          this.registers[i] = this.getRegister(String(i - 1));
        }
      },
    };
    var commandDispatcher = {
      matchCommand(key, keyMap, vim) {
        ...
        const keys = inputState.keyBuffer.concat(key);
        ...
        if (!matchedCommands.length) {
          ...
          const context = vim.visualMode ? 'visual' : 'normal';
          let bestMatch; // Default to first in the list.

          for (var i = 0; i < matchedCommands.length; i++) {
            const current = matchedCommands[i];
      ...
      processAction(cm, vim, command) {
        ...
        const repeatIsExplicit = Boolean(repeat);
      ...
      evalInput(cm, vim) {
        ...
        let selectionEnd = cm.getCursor('head');
        let selectionStart = cm.getCursor('anchor'); // The difference between cur and selection cursors are that cur is
        // being operated on and ignores that there is a selection.

        let curStart = copyCursor(selectionEnd);
        const curOriginal = copyCursor(curStart);
        let curEnd;
        let repeat;
        ...
        if (motion) {
          const motionResult = motions[motion](cm, motionArgs, vim);
          ...
          if (motionArgs.toJumplist) {
            const jumpList = vimGlobalState.jumpList; // if the current motion is # or *, use cachedCursor
    ...
    var motions = {
      ...
      findNext(cm, motionArgs) {
        ...
        let prev = !motionArgs.forward; // If search is initiated with ? instead of /, negate direction.
      ...
      moveToOtherHighlightedEnd(cm) {
        const curEnd = copyCursor(cm.getCursor('head'));
        const curStart = copyCursor(cm.getCursor('anchor'));
      ...
      moveByLines(cm, motionArgs, vim) {
        const cur = cm.getCursor();
        let endCh = cur.ch; // Depending what our last motion was, we may want to do different
        ...
        const line = motionArgs.forward ? cur.line + repeat : cur.line - repeat;
        const first = cm.firstLine();
        const last = cm.lastLine(); // Vim cancels linewise motions that start on an edge and move beyond
      ...
      moveByDisplayLines(cm, motionArgs, vim) {
        const cur = cm.getCursor();
      ...
      moveByPage(cm, motionArgs) {
        ...
        const curStart = cm.getCursor();
        const repeat = motionArgs.repeat;
        cm.moveV(motionArgs.forward ? repeat : -repeat, 'page');
        const curEnd = cm.getCursor();
      ...
      moveByParagraph(cm, motionArgs) {
        ...
        const repeat = motionArgs.repeat;
        const inc = motionArgs.forward ? 1 : -1;
      ...
      moveByScroll(cm, motionArgs, vim) {
        ...
        const orig = cm.charCoords(cm.getCursor(), 'local');
        motionArgs.repeat = repeat;
      ...
      moveByWords(cm, motionArgs) {
        return moveToWord(
          cm,
          motionArgs.repeat,
          Boolean(motionArgs.forward),
          Boolean(motionArgs.wordEnd),
          Boolean(motionArgs.bigWord),
        );
      },
      ...
      moveToColumn(cm, motionArgs, vim) {
        const repeat = motionArgs.repeat; // repeat is equivalent to which column we want to move to!
      ...
      moveToMatchedSymbol(cm) {
        ...
        let symbol;
        const startContext = cm.getTokenAt(cursor).type;
        const startCtxLevel = getContextLevel(startContext);

        do {
          symbol = lineText.charAt(ch++);

          if (symbol && isMatchableSymbol(symbol)) {
            const endContext = cm.getTokenAt({
              line,
              ch,
            }).type;
            const endCtxLevel = getContextLevel(endContext);
      ...
      textObjectManipulation(cm, motionArgs) {
        ...
        const character = motionArgs.selectedCharacter; // Inclusive is the difference between a and i
    ...
    var operators = {
      change(cm, operatorArgs, _vim, curStart, curEnd) {
        ...
        if (operatorArgs.linewise) {
          ...
          const replacement = curEnd.line > cm.lastLine() ? '' : '\n';
          cm.replaceRange(replacement, curStart, curEnd);
          cm.indentLine(curStart.line, 'smart'); // null ch so setCursor moves to end of line.

          curStart.ch = null;
        } else {
          // Exclude trailing whitespace if the range is not all whitespace.
          const text = cm.getRange(curStart, curEnd);
      ...
      indent(cm, operatorArgs, vim, curStart, curEnd) {
        const startLine = curStart.line;
        let endLine = curEnd.line; // In visual mode, n> shifts the selection right n times, instead of
        // shifting n lines right once.

        const repeat = vim.visualMode ? operatorArgs.repeat : 1;
      ...
      swapcase(cm, operatorArgs, _vim, curStart, curEnd, curOriginal) {
        const toSwap = cm.getRange(curStart, curEnd);
        let swapped = '';
    ...
    var actions = {
      ...
      scrollToCursor(cm, actionArgs) {
        ...
        const charCoords = cm.charCoords(
          {
            line: lineNum,
            ch: 0,
          },
      ...
      toggleVisualMode(cm, actionArgs, vim) {
        ...
        let curEnd; // TODO: The repeat should actually select number of characters/lines
        ...
        if (!vim.visualMode) {
          cm.on('mousedown', exitVisualMode);
          vim.visualMode = true;
          vim.visualLine = Boolean(actionArgs.linewise);
      ...
      replace(cm, actionArgs, vim) {
        ...
        if (vim.visualMode) {
          ...
          curEnd = cm.getCursor('end'); // workaround to catch the character under the cursor
        ...
        if (replaceWith == '\n') {
          ...
          let replaceWithStr = cm.getRange(curStart, curEnd); // replace all characters in range by selected, but keep linebreaks
    ...
    function exitVisualMode(cm) {
      ...
      const vim = cm.state.vim; // can't use selection state here because yank has already reset its cursor
    ...
    function clipToLine(cm, curStart, curEnd) {
      const selection = cm.getRange(curStart, curEnd); // Only clip if the selection ends with trailing newline + whitespace
    ...
    function expandWordUnderCursor(cm, inclusive, _forward, bigWord, noSymbol) {
      ...
      let idx = cur.ch; // Seek to first word or non-whitespace character, depending on if
      // noSymbol is true.

      let textAfterIdx = line.substring(idx);
      let firstMatchedChar;
      ...
      const textBeforeIdx = line.substring(0, idx);
      let matchRegex; // Greedy matchers for the "word" we are trying to expand.
      ...
      const wordAfterRegex = matchRegex.exec(textAfterIdx);
      let wordStart = idx;
      let wordEnd = idx + wordAfterRegex[0].length; // TODO: Find a better way to do this. It will be slow on very long lines.

      const revTextBeforeIdx = reverse(textBeforeIdx);
      const wordBeforeRegex = matchRegex.exec(revTextBeforeIdx);
      ...
      if (inclusive) {
        ...
        const textAfterWordEnd = line.substring(wordEnd);
        const whitespacesAfterWord = textAfterWordEnd.match(/^\s*/)[0].length;

        if (whitespacesAfterWord > 0) {
          wordEnd += whitespacesAfterWord;
        } else {
          const revTrim = revTextBeforeIdx.length - wordStart;
          const textBeforeWordStart = revTextBeforeIdx.substring(revTrim);
          const whitespacesBeforeWord = textBeforeWordStart.match(/^\s*/)[0]
            .length;
    ...
    function findWord(cm, cur, forward, bigWord, emptyLineIsWord) {
      ...
      const regexps = bigWord ? bigWordRegexp : wordRegexp;
      ...
      while (true) {
        ...
        let wordStart = stop;
        let wordEnd = stop; // Find bounds of next word.

        while (pos != stop) {
          let foundWord = false;

          for (let i = 0; i < regexps.length && !foundWord; ++i) {
    ...
    function findMatchedSymbol(cm, cur, symb) {
      ...
      const symbContext = cm.getTokenAt({
        line,
        ch: ch + 1,
      }).type;
      const symbCtxLevel = getContextLevel(symbContext);
      const reverseSymb = {
        '(': ')',
        ')': '(',
        '[': ']',
        ...
        '{': '}',
        '}': '{',
      }[symb]; // Couldn't find a matching symbol, abort
      ...
      const endLine = increment === 1 ? cm.lineCount() : -1;
      let depth = 1;
      let nextCh = symb;
      let index = ch;
      let lineText = cm.getLine(line); // Simple search for closing paren--just count openings and closings till
      // we find our match
      ...
      while (line !== endLine && depth > 0) {
        ...
        const revSymbContext = cm.getTokenAt({
          line,
          ch: index + 1,
        }).type;
        const revSymbCtxLevel = getContextLevel(revSymbContext);
    ...
    function selectCompanionObject(cm, revSymb, inclusive) {
      const cur = cm.getCursor();
      let end = findMatchedSymbol(cm, cur, revSymb);
      let start = findMatchedSymbol(cm, end);
    ...
    function findBeginningAndEnd(cm, symb, inclusive) {
      ...
      let i;
      let len;
      const firstIndex = chars.indexOf(symb); // the decision tree is to always look backwards for the beginning first,
    ...
    function translateRegex(str) {
      ...
      const specials = ['|', '(', ')', '{']; // Remove, but never add, a '\' for these.
      ...
      for (let i = -1; i < str.length; i++) {
        ...
        let specialComesNext = specials.indexOf(n) != -1;
    ...
    function highlightSearchMatches(cm, query) {
      let overlay = getSearchState(cm).getOverlay();
    ...
    Vim.ExCommandDispatcher.prototype = {
      processCommand(cm, input) {
        ...
        const inputStream = new CodeMirror.StringStream(input); // update ": with the latest command whether valid or invalid
        ...
        if (!params.commandName) {
          ...
          const command = this.matchCommand_(params.commandName);
    ...
    var exCommands = {
      ...
      set(cm, params) {
        ...
        if ((!optionIsBoolean && !value) || forceGet) {
          const oldValue = getOption(optionName); // If no value is provided, then we assume this is a get.
      ...
      substitute(cm, params) {
        ...
        const slashes = argString ? findUnescapedSlashes(argString) : [];
        let replacePart = '';
        ...
        if (slashes.length) {
          ...
          if (slashes[2]) {
            ...
            // by count.
            const trailing = argString.substring(slashes[2] + 1).split(' ');
        ...
        const startPos = clipCursorToContent(cm, {
          line: lineStart,
          ch: 0,
        });
      ...
      delmarks(cm, params) {
        ...
        while (!stream.eol()) {
          ...
          const sym = stream.next(); // Check if this symbol is part of a range
          ...
          if (stream.match('-', true)) {
            ...
            const startMark = sym;
            const finishMark = stream.next(); // The range must terminate at an alphabetic character which
    ...
    function buildVimKeyMap() {
      ...
      function bindKeys(keys, modifier) {
        for (let i = 0; i < keys.length; i++) {
          ...
          const vimKey = cmKeyToVimKey(keys[i], modifier);
          const cmKey = modifier ? modifier + '-' + key : key;
    ...
    function exitInsertMode(cm) {
      ...
      insertModeChangeRegister.setText(
        macroModeState.lastInsertModeChanges.changes.join(''),
      );
      CodeMirror.signal(cm, 'vim-mode-change', {
        mode: 'normal',
      });
    ...
    function repeatLastEdit(cm, vim, repeat, repeatForInsert) {
      const macroModeState = vimGlobalState.macroModeState;
      macroModeState.isPlaying = true;
      const isAction = Boolean(vim.lastEditActionCommand);
      ...
      function repeatInsert(repeat) {
        if (macroModeState.lastInsertModeChanges.changes.length > 0) {
          ...
          const changeObject = macroModeState.lastInsertModeChanges; // This isn't strictly necessary, but since lastInsertModeChanges is

  ...
  var Vim = function() {
    ...
    var commandDispatcher = {
      ...
      evalInput(cm, vim) {
        ...
        let selectionStart = cm.getCursor('anchor');
    ...
    var motions = {
      moveToTopLine(cm, motionArgs) {
        ...
        return { line, ch: findFirstNonWhiteSpaceCharacter(cm.getLine(line)) };
      },
      moveToMiddleLine(cm) {
      ...
      moveToMatchedSymbol(cm) {
        ...
        do {
          ...
          if (symbol && isMatchableSymbol(symbol)) {
            const endContext = cm.getTokenAt({ line, ch }).type;
        ...
        if (symbol) {
          return findMatchedSymbol(cm, { line, ch: ch - 1 }, symbol);
        } else {
          return cursor;
        }
      },
    ...
    var actions = {
      ...
      scrollToCursor(cm, actionArgs) {
        ...
        const charCoords = cm.charCoords({ line: lineNum, ch: 0 }, 'local');
      ...
      incrementNumberToken(cm, actionArgs) {
        ...
        if (token) {
          ...
          const from = { ch: start, line: cur.line };
          const to = { ch: end, line: cur.line };
    ...
    function expandWordUnderCursor(cm, inclusive, _forward, bigWord, noSymbol) {
      ...
      let wordStart = idx;
      let wordEnd = idx + wordAfterRegex[0].length;
    ...
    function findMatchedSymbol(cm, cur, symb) {
      ...
      const symbContext = cm.getTokenAt({ line, ch: ch + 1 }).type;
      ...
      while (line !== endLine && depth > 0) {
        ...
        const revSymbContext = cm.getTokenAt({ line, ch: index + 1 }).type;
    ...
    var exCommands = {
      ...
      set(cm, params) {
        ...
        if ((!optionIsBoolean && !value) || forceGet) {
          const oldValue = getOption(optionName);
    ...
    function buildVimKeyMap() {
      ...
      function cmKeyToVimKey(key, modifier) {
        ...
        const specialKey = { Enter: 'CR', Backspace: 'BS', Delete: 'Del' }[
          vimKey
        ];
